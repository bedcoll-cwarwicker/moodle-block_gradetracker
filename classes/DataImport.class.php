...
class DataImport {
    ...
    public function checkFileUnitDataSheet()
    {
        ...
        // See if anything has been updated in the DB since we downloaded the file
        $updates = $DB->get_records_sql("SELECT uc.*
                                        FROM {bcgt_user_criteria} uc
        ...
        if ($updates)
        {
            ...
            $output .= "<div class='gt_import_warning'>";
                ...
                foreach($updates as $update)
                {
                    ...
                    $value = new \GT\CriteriaAward($update->awardid);
                    $output .= sprintf( get_string('aupdatedtobbycatd', 'block_gradetracker'), "{$unit->getDisplayName()} ({$criterion->getName()})", $value->getName(), $updateBy->getDisplayName(), date('d-m-Y, H:i', $update->lastupdate)) . "<br>";
        ...
        $output .= "<div class='gt_import_grid_div'>";
            ...
            $output .= "<input type='hidden' name='now' value='{$now}' />";
            $output .= "<input type='submit' class='gt_btn gt_btn_small' name='confirm' value='".get_string('confirm')."' />";
            $output .= str_repeat("&nbsp;", 8);
            $output .= "<input type='button' class='gt_btn gt_btn_small' onclick='window.location.href=\"{$CFG->wwwroot}/blocks/gradetracker/grid.php?type=student&id={$this->getStudentID()}&qualID={$this->getQualID()}\";' value='".get_string('cancel')."' />";
    ...
    public function checkFileClassDataSheet()
    {
        ...
        else
        {
            ...
            for($sheetNum = 0; $sheetNum < $cntSheets; $sheetNum++)
            {
                ...
                // See if anything has been updated in the DB since we downloaded the file
                $updates = $DB->get_records_sql("SELECT uc.*, usr.firstname, usr.lastname
                                                FROM {bcgt_user_criteria} uc
                                                INNER JOIN {bcgt_criteria} c ON c.id = uc.critid
