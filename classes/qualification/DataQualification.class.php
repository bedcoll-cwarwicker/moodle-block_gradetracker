...
class DataQualification extends \GT\Qualification {
    ...
    public function getQualificationReportStudents($unitAwards, $view, $criteriaNames, $shortCriteriaNames, $courseID = false){
        ...
        $sql = "SELECT DISTINCT 
                ...
                tg.name as targetgrade, tg.id as targetgradeid,
                wtg.name as weightedtargetgrade, wtg.id as weightedtargetgradeid,
                ag.name as aspirationalgrade, ag.id as aspirationalgradeid,
                cg.name as cetagrade,
        ...
        if ($view == 'view-criteria-short' && $shortCriteriaNames)
        {
            ...
            foreach($shortCriteriaNames as $name)
            {
                ...
                $sql .= "LEFT JOIN ( 
                            ...
                            WHERE uqu.qualid = ? AND a.met = 1 AND c.name LIKE ? AND c.deleted = 0
                            GROUP BY uc.userid
        ...
        $sql .= "
                LEFT JOIN ( 
                    ...
                    INNER JOIN {bcgt_qual_units} qu ON (qu.qualid = uqu.qualid AND qu.unitid = uqu.unitid)
                    INNER JOIN {bcgt_criteria_awards} a ON a.id = uc.awardid
                    WHERE uqu.qualid = ? AND a.met = 1 AND c.deleted = 0
                    GROUP BY uc.userid
