...
//        $courseID = ($this->course && $this->course->isValid()) ? $this->course->id : false;
                        ...
                        if ($col != 'A' && $col != 'B' && $col != 'C' && $col != 'D'){
                            ...
                            if ($studentCriterion)
                            {
                                ...
                                if (array_key_exists($value, $possibleValueArray) !== false || $value == $naValueObj->getShortName())
                                {
                                    ...
                                    $noEvent = ($col == $objWorksheet->getHighestColumn()) ? false : true;
    ...
    public function import($file = false)
    {
        ...
        // Log the action first, otherwise it will appear after all the criteria logs
        // All the criteria will have their own logs, because it uses saveUser()
        $Log = new \GT\Log();
        $Log->addLog(\GT\Log::GT_LOG_CONTEXT_GRID, \GT\Log::GT_LOG_DETAILS_IMPORTED_GRID, array(
            "attributes" => array(
                "type" => "student",
