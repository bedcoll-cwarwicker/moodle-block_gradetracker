...
class GradeTracker
{
    ...
    public function displayConfigReporting($TPL, $section, $page, $id)
    {
        ...
        if ($section == 'query')
        {
            ...
            if ($page == false){
                $queries = $DB->get_records_sql("SELECT *
                         FROM {bcgt_query_reporting}", array());
                $TPL->set("queries", $queries);
            }

            if (isset($id)) {
                $query = $DB->get_record_sql("SELECT *
                         FROM {bcgt_query_reporting}
                         WHERE id = ?", array($id));

                $TPL->set("query", $query);
            }

            if ($page == "run" && $query && gt_has_capability('block/gradetracker:run_sql_report')){

                try
                {
                    $results = $DB->get_records_sql($query->query, array());

                    $query->last_ran = time();
                    $DB->update_record('bcgt_query_reporting', $query, false);
                    ...
                    $MSGS['errors'][] = get_string('errors:sql:query', 'block_gradetracker') . $e->getMessage() . ' (' . $e->error . ')';
                    $TPL->set("MSGS", $MSGS);
                }
    ...
    public function saveConfig($view, $section, $page)
    {
        ...
        switch($view)
        {
            ...
            case 'settings':
                ...
                $this->saveConfigSettings($section, $page);
        ...
        // Log the action
        if ($section != 'search' && $section != 'logs'){
            
            $queryStringParams = array();
            parse_str($_SERVER['QUERY_STRING'], $queryStringParams);
            $queryStringParams['POST'] = json_encode($_POST);
                        
            $Log = new \GT\Log();
            $Log->addLog(\GT\Log::GT_LOG_CONTEXT_CONFIG, \GT\Log::GT_LOG_DETAILS_SAVED_CONFIG, array(
    ...
    private function saveConfigUnits($section, $page)
    {
        ...
        if ($section == 'new' && !isset($_POST['restoreUnit']))
        {
        ...
        {   
            ...
            $results = $GUI->submitFormUnitSearch();
            $TPL->set("results", $results);
            
            $deletedresults = $GUI->submitFormUnitSearch(true);
        ...
        if ((isset($_POST['restoreUnit_x']) || isset($_POST['restoreUnit'])) && gt_has_capability('block/gradetracker:delete_restore_units'))
        {
            $restoreUnit = new \GT\Unit($id);
            $restoreUnit->restore();
    ...
    private function saveConfigQuals($section, $page)
    {
        ...
        if(isset($_POST['copy_qual'])){
            $qual = new \GT\Qualification($id);
            $qual->copyQual();
        }
        
        // New Qualification submission
        if ($section == 'new' && !isset($_POST['restoreQual']))
        {
            
            $TPL = new \GT\Template();
            $GUI = new \GT\Qualification\GUI($id);
            $GUI->loadTemplate($TPL);
            $GUI->saveFormNewQualification();
        ...
        elseif ($section == 'search' && isset($_POST['submit_search']))
        {
            ...
            $results = $GUI->submitFormSearch();
            $TPL->set("results", $results);
            
            $deletedresults = $GUI->submitFormSearch(true);
            $TPL->set("deletedresults", $deletedresults);
        ...
        {
            $restoreQual = new \GT\Qualification($id);
            $restoreQual->restore();
...
//            }
    ...
    private function saveConfigReportingLogs(){
        ...
        if (isset($_POST['search']))
        {
            ...
            if (isset($_POST['timing']) && $_POST['timing'] != ''){
                $params['timing'] = $_POST['timing'];
            }
            
            if (isset($_POST['date']) && trim($_POST['date']) != ''){
                $params['dateStart'] = strtotime(trim($_POST['date']));
                $params['dateEnd'] = strtotime(trim($_POST['date'] . ' 23:59:59'));
            }
            
            if (isset($_POST['user']) && trim($_POST['user']) != ''){
                $params['user'] = trim($_POST['user']);
            }
    ...
    private function saveConfigReportingQuery()
    {
        
        global $DB, $VARS, $MSGS, $TPL;
        
        if (isset($_POST['delete_query']) && gt_has_capability('block/gradetracker:crud_sql_report'))
        {
            ...
            $query->deleted = 1;
            $lastinsertid = $DB->update_record('bcgt_query_reporting', $query, false);
            
        }
        
        
        if (isset($_POST['submit_new_query']) && gt_has_capability('block/gradetracker:crud_sql_report'))
        {
            $sql = trim($_POST['query']);            
            
            if (!\GT\Reports\Report::checkContainsDisallowedWords($sql)) {
                ...
                $record->last_edited = $created_date;
                $lastinsertid = $DB->insert_record('bcgt_query_reporting', $record, false);
                $MSGS['success'] = get_string('querysaved', 'block_gradetracker');
                
                // Override "page" variable to load the list instead
                $TPL->set("page", false, true);
                
            }
            else {
                $MSGS['errors'] = sprintf( get_string('errors:sql:disallowed', 'block_gradetracker'), implode(', ', \GT\Reports\Report::getDisallowedWords()) );
            }
            
        }
        
        if (isset($_POST['submit_edit_query']) && gt_has_capability('block/gradetracker:crud_sql_report'))
        {
            ...
            if (!\GT\Reports\Report::checkContainsDisallowedWords($sql)) {
                ...
                $record = new \stdClass();
                $record->id = $_POST['query_id'];
                $record->name = $_POST['name'];
                $record->description = $_POST['description'];
                $record->query = $sql;
                $record->created_date = $_POST['created_date'];
                $record->last_edited = time();
                $record->last_ran = $_POST['last_ran'];
                $lastinsertid = $DB->update_record('bcgt_query_reporting', $record, false);
                $MSGS['success'] = get_string('querysaved', 'block_gradetracker');
            }
            else {
                $MSGS['errors'] = sprintf( get_string('errors:sql:disallowed', 'block_gradetracker'), implode(', ', \GT\Reports\Report::getDisallowedWords()) );
...
//            '/blocks/gradetracker/js/jquery/css/ui-lightness/theme.css',
    ...
    public static function gc($dir = false){
